...
- 本文按 `Recast/Include` 和 `Recast/Source` 的当前源码核对，重点检查 rasterization、filter、compact heightfield、area marking、distance field、watershed/monotone/layer regions、contour、poly mesh、detail mesh 和 heightfield layers。
...
- https://groups.google.com/g/recastnavigation/c/XIjvm79w0U4
- https://groups.google.com/g/recastnavigation/c/s3s2xMi5TTI
- https://groups.google.com/g/recastnavigation/c/m_JMXgmlIs4
...
6. **坐标系假设贯穿整个构建流程。**
   ...
   Recast 的体素网格是 xz 平面网格，y 是高度方向。`rcMarkWalkableTriangles()` 看三角形法线的 y 分量；heightfield cell 用 `(x,z)` 定位；`rcCompactSpan::con` 四方向也是 xz 平面邻接。官方社区讨论明确说明，Y-up 右手坐标和直立角色假设已经深入库内部。想改成 YZ、墙面或天花板导航，不是替换几个轴名就能完成。
...
`walkableHeight` 和 `walkableClimb` 必须按 `ceil(worldHeight / ch)` 或类似方式转成体素高度。`ch` 太大时，台阶、低矮空间和坡面都会变得粗糙。
...
Layer 分区用于构建可分层的 heightfield layers，常和 tile cache 相关。
...
输出的 `reg` 更适合后续 `rcBuildHeightfieldLayers()` 生成 2D layer。
...
默认只开启 wall edges。原因是墙边过长会影响后续 tile 边界和轮廓贴合；area 边界是否需要细分取决于应用是否需要更精细地保留语义边界。源码通过顶点第 4 分量中的 `RC_CONTOUR_REG_MASK`、`RC_AREA_BORDER`、`RC_BORDER_VERTEX` 判断一条 raw contour segment 属于哪类边。
