...
- 本文按仓库提交 `9f4ce64` 的 `Detour/Include` 和 `Detour/Source` 逐段对齐，既核对公开 API，也核对其背后的内部状态。重点包括 `dtCreateNavMeshData()`、tile 装卸和状态保存、引用校验、A*/sliced A*、funnel、BV 查询、局部移动、raycast 和局部 Dijkstra。
...
- [Detour 官方模块文档](https://recastnav.com/group__detour.html)
- [`dtNavMesh` 官方 API](https://recastnav.com/classdtNavMesh.html)
- [`dtNavMeshQuery` 官方 API](https://recastnav.com/classdtNavMeshQuery.html)
- [官方 Introduction：polygon corridor 与 string pulling](https://recastnav.com/md_Docs_2__1__Introduction.html)
- [官方集成文档](https://recastnav.com/md_Docs_2__2__BuildingAndIntegrating.html)
- [官方论坛：查询数据与查询状态分离、并行查询的由来](https://groups.google.com/g/recastnavigation/c/r7gL4F552m4)
- [官方论坛：不同 up axis 与墙面/天花板导航限制](https://groups.google.com/g/recastnavigation/c/43XlbrGVIdM)

源码覆盖关系：

| 源码 | 本文对应内容 |
| --- | --- |
| `DetourNavMeshBuilder.cpp` | tile data 的布局、BV tree、detail fallback、off-mesh 编码和端序转换 |
| `DetourNavMesh.cpp` | 初始化、tile hash/free list、内部/外部/off-mesh link、卸载、salt、tile state 和运行时 poly 状态 |
| `DetourNavMeshQuery.cpp` | filter、A*、sliced/any-angle、funnel、空间查询、局部移动、raycast、Dijkstra、随机点和墙段 |
| `DetourNode.cpp` | 固定节点池、hash 查找、二叉堆开放列表 |
| `DetourCommon.cpp` | 上述算法依赖的 2D 相交、点到边距离、凸多边形采样和高度插值；本文在使用处解释，不逐个复述向量辅助函数 |

## 一、整体设计

`Detour` 把导航网格拆成多个 tile，每个 tile 内包含若干凸多边形。运行时查询时，`Detour` 不直接在三角形上搜索，而是在“多边形图”上搜索：

![Detour 从 Recast 构建结果到运行时查询的完整数据流](https://oss.euler.icu/teaser/recastnavigation/Detour/Docs/Images/detour-runtime-pipeline.png)

*图 1：Recast 输出被打包为连续 tile data，加载后形成可查询的多边形拓扑，并进一步产生 corridor、拉直路径和表面射线结果。*
...
![Detour 身份、拓扑、几何和语义四层模型](https://oss.euler.icu/teaser/recastnavigation/Detour/Docs/Images/detour-four-layer-model.png)

*图 2：四层数据分别解决稳定引用、图连通、表面几何和通行语义问题，查询阶段再把它们组合起来。*
...
![dtPolyRef 的 salt、tile 和 poly 编码及失效过程](https://oss.euler.icu/teaser/recastnavigation/Detour/Docs/Images/detour-polyref-lifecycle.png)

*图 3：tile 槽位复用后 salt 会递增，旧引用即使仍包含相同槽位下标，也会在校验阶段被判定为 stale。*
...
![dtNavMesh addTile 的六个关键阶段](https://oss.euler.icu/teaser/recastnavigation/Detour/Docs/Images/detour-add-tile.png)

*图 4：tile data 经校验和槽位分配后恢复内部指针，再建立 link free list、内部连接、跨 tile portal 和 off-mesh 连接。*
...
`setAreaCost()` 不做范围校验。默认 A* 启发式约等于直线距离，因此要保持它的最优性假设，常规 area cost 应为有限非负值且最好不小于 1；若允许小于 1，启发式可能高估真实剩余成本，搜索仍能返回可行 corridor，但不再能据此保证代价最优。负 cost 会破坏 A*/Dijkstra 的基本前提，应在应用配置层禁止。
...
当前源码中 `H_SCALE = 0.999f`。启发式是到终点的欧氏距离。略小于 1 使它保持保守，减少浮点误差把理论上等价的路径误判成更优的风险，同时仍保留 A* 的方向性。

输入校验确认 start/end ref 存在，但不会先对 `startRef` 调 `passFilter()`；`startRef == endRef` 时还会直接返回该 ref。正常调用应先通过 `findNearestPoly()` 或 `isValidPolyRef(ref, filter)` 得到符合角色 filter 的起点。否则 A* 可能从一个按当前规则本不应站立的 polygon 开始，再只对后续邻居应用 filter。

### 3. 主循环

每轮：

![Detour 在 polygon corridor 上执行 A 星搜索并维护 open heap](https://oss.euler.icu/teaser/recastnavigation/Detour/Docs/Images/detour-astar-search.png)

> **图 5：`dtNavMeshQuery::findPath()` 的 A* 搜索。** 左侧青色 polygon 是最终 corridor，黄色与蓝色分别表示 open/closed 集合；node 位置通常取进入该 polygon 的 portal 中点，而非 polygon 中心。右侧对应源码主循环：从二叉堆弹出最小 `total`，沿 `dtLink` 枚举邻居并执行 `dtQueryFilter`，以 `cost + 0.999 * heuristic` 比较候选，随后更新 `pidx/cost/total` 并 `modify` 或 `push` 堆。若 open list 先耗尽，则从 `lastBestNode` 回溯并返回带 `DT_PARTIAL_RESULT` 的 corridor。
...
节点池的 key 实际是 `(polyRef, state)`。普通内部边的 `state` 为 0；跨 tile link 会由 `side >> 1` 得到进入方向状态。这样同一个 polygon 可以保留从不同 tile side 进入的候选节点，避免把几何上差异很大的入口过早合并成一个 A* 状态。节点池容量不足时，搜索不会扩容，而是跳过无法创建的节点并在返回值中附加 `DT_OUT_OF_NODES`。
...
![Funnel 算法从 portal corridor 中收紧边界并产生转角](https://oss.euler.icu/teaser/recastnavigation/Detour/Docs/Images/detour-funnel-algorithm.png)

*图 6：左右边界持续收紧；当一侧跨过另一侧时，必须输出对侧边界作为转角，并从新的 apex 继续处理后续 portal。*
...
它的“局部”还体现在源码硬边界上：待处理 stack 最多 48 个节点，每条外部边最多暂存 8 个邻居，并复用 query 的 tiny node pool。超出 stack/node/邻居容量时实现会停止加入更多候选，而不会把它升级成全局 A* 或完整报告 `DT_OUT_OF_NODES`。因此 `endPos` 应是每帧附近目标；把很远终点交给它，得到的是受局部预算约束的最近合法点，而不是全局可达性结论。
...
新版重载把结果写入 `dtRaycastHit`，其中 `path/pathCount` 是穿过的 corridor，`pathCost` 只在传入 `DT_RAYCAST_USE_COSTS` 时按 `dtQueryFilter::getCost()` 累积。`prevRef` 用于给第一段 cost 提供前驱上下文。命中墙时 `t` 位于 `[0,1]`；到达终点而未撞墙时源码以 `FLT_MAX` 表示“无墙命中”。因此调用方不能只用 `t == 1` 判断成功。
...
`dtStatus` 的高位表示主状态 `DT_FAILURE / DT_SUCCESS / DT_IN_PROGRESS`，低位可以同时携带 `DT_WRONG_MAGIC`、`DT_WRONG_VERSION`、`DT_OUT_OF_MEMORY`、`DT_INVALID_PARAM`、`DT_BUFFER_TOO_SMALL`、`DT_OUT_OF_NODES`、`DT_PARTIAL_RESULT` 等细节。因此“成功”不等于“完整”：调用者应先用 `dtStatusFailed()` 判断主状态，再根据业务检查 buffer、node 和 partial 标志。
...
`dtNavMesh` 保存共享的 tile 几何与拓扑；`dtNavMeshQuery` 保存 node pool、open list、sliced query 和最近一次 Dijkstra 的可变状态。官方论坛中关于并行查询的讨论正是促成两者分离的背景。工程上的并发边界是：

1. navmesh 不发生 add/remove tile 或状态写入时，多个线程可以只读共享同一个 `dtNavMesh`。
2. 每个并发查询执行流必须拥有独立 `dtNavMeshQuery`，不能并发复用同一个 query。
3. sliced query 从 `init` 到 `finalize` 独占该 query；期间调用其他会复用 node pool 的查询会破坏它。
4. tile 流式更新、TileCache 重建、`setPolyFlags()`、`setPolyArea()` 等写操作要由应用层与查询同步；库本身不提供锁。

### 管理和辅助 API 的边界

- `init(params)` 建立 tiled navmesh 的固定槽位、位置哈希和 ref bit 布局；`init(data, size, flags)` 是 single-tile 便利入口，内部仍走 tile 机制。
- `addTile()` / `removeTile()` 管理 tile data 所有权。只有带 `DT_TILE_FREE_DATA` 的 tile 会由 navmesh 释放；否则 remove 可把原始 data/dataSize 交还调用方。
- `calcTileLoc()`、`getTilesAt()`、`getTileAt()`、`getTileRefAt()`、`getTileByRef()`、`getTileRef()` 负责世界坐标、tile 坐标、layer、指针与稳定引用之间的映射；同一 `(x,y)` 可以有多个垂直 layer。
- `getPolyRefBase(tile)` 返回该 tile 的 poly 0 引用，内部热路径可与 tile 内 poly index 按位组合；它依赖当前 salt，tile 重载后必须重新取得。
- `getTileAndPolyByRef()` 做完整 salt、tile、poly 范围检查；`getTileAndPolyByRefUnsafe()` 只适合已经校验过的内部热路径。
- `getOffMeshConnectionByRef()` 返回连接元数据；`getOffMeshConnectionPolyEndPoints(prevRef, polyRef, ...)` 根据进入 off-mesh polygon 的前驱 ref 决定端点方向。同一条双向连接从两侧进入时，返回的 start/end 会相反。
- `closestPointOnPoly()`、`closestPointOnPolyBoundary()` 和 `getPolyHeight()` 分工不同：前者会返回表面最近点，boundary 版本只在 xz 边界外夹点，height 版本要求查询点位于 polygon 的 xz 范围内并使用 detail mesh 求 y。

Detour 对象应通过 `dtAllocNavMesh()` / `dtFreeNavMesh()`、`dtAllocNavMeshQuery()` / `dtFreeNavMeshQuery()` 创建和销毁，才能和 `dtAllocSetCustom()` 设置的 allocator 保持一致。直接混用另一套 allocator 会破坏 tile/query 内部内存的释放责任。
