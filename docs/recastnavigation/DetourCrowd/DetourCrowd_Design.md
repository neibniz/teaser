...
- 本文按 `DetourCrowd/Include` 和 `DetourCrowd/Source` 的当前源码核对，重点检查 `dtCrowd::update()`、`checkPathValidity()`、`updateMoveRequest()`、`updateTopologyOptimization()`、`dtPathCorridor` 三种合并、`dtPathQueue`、`dtObstacleAvoidanceQuery` 和 `dtLocalBoundary`。
...
- https://recastnav.com/classdtCrowd.html
- https://recastnav.com/md_Docs_2__2__BuildingAndIntegrating.html
- https://recastnav.com/md_Docs_2__99__Roadmap.html
...
```text
全局路径：低频维护 polygon corridor
局部运动：每帧根据 corridor corner、邻居 agent、墙段采样安全速度
位置约束：运动后用 moveAlongSurface 回到 navmesh，并修正 corridor
```
...
```text
路径层：target -> path corridor -> corners
意图层：corners -> desired velocity
避障层：desired velocity + obstacles -> safe sampled velocity
运动层：acceleration limit -> integrated position
修正层：collision projection + moveAlongSurface -> valid navmesh position
维护层：visited polys -> merge corridor -> next frame
```
...
`update(maxIters)` 每帧最多消耗一定 A* 迭代数。请求成功后结果不会立刻丢弃，而是 keepAlive 几帧，给 crowd 读取结果的机会。
...
更新局部墙段和邻居 agent
  |
  v
提取 corridor corners，可视优化
  |
  v
触发 off-mesh connection
  |
  v
计算期望速度 dvel
  |
...
多轮重叠碰撞解算
  |
  v
moveAlongSurface 约束回 navmesh，修正 corridor
  |
  v
更新 off-mesh 动画
```
...
真实项目一般会用这个状态触发跳跃或爬梯动画，然后把动画 root motion 回写给 crowd 或在结束时同步位置。
